...
def panda_plus_symmetry_generate(): #function for calling from master file
    ...
    for i in range(final_Num_Trees):  # create the filters a put them into a filter array
        tree_filter = (distances_unfiltered["Num_First_tree"] == i) & (distances_unfiltered[
                                                                           "Num_Second_tree"] <= final_Num_Trees)  # tree filter is collection of boolean values literally a filter filters the trees into boolean values
