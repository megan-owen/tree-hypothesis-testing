...
def crossmatchdist(bigN,n):
    ...
    dist = pd.DataFrame(index = ["a0", "a1", "a2", "pr"]) #creating a DataFrame with indices "a0", "a1", "a2", and "pr".
    
    for a1 in range(0,I+1): #loop for possible values of a1
            a2 = (n-a1)/2
            if ((math.floor(a2)==a2)&(a2>=0)):
                    int_a2 = int(a2) #converting a2
                    a0 = I-(a1+int_a2)
                    
# Notice that a0 = I-(a1+int_a2) = I-(a1 + (n-a1)/2) = I - ( 2a1 + n - a1 )/2 = I - (n+ a1)/2 

                    if (a0>=0): # Check if a0 is non-negative.
    ...
    return dist.to_numpy() # Return the DataFrame as a numpy array
...
def crossmatchdist_df(bigN,n):
    if (bigN%2 == 1):
        return "The number of subjects, bigN, should be even"
    I = int(bigN/2)
    dist = pd.DataFrame(index = [1,2,3,4]) #creates a dataframe with indices 1-4
...
def crossmatchtest(z,D):
    if (not check_symmetric(D)): # Check if the distance matrix is symmetric
    ...
    G = nx.from_numpy_array(D) #  Create a graph from the distance matrix
    ...
    if(len(list1)>0): #for matched pairs
    ...
    if(bigN < 340):        
            ...
            for j in range(len(dist[1])):
                if(dist[1][j] == a1):
                    pval = dist[3][j] #p-value, should be from 0 to 1
    ...
    dev = (a1-Ea1)/math.sqrt(Va1) #observed difference 
    ...
    final_list = {'a1':a1,
                 'Ea1':Ea1, 
                  'Va1':Va1,
                  'dev':dev,
                  'pval':pval, #reject null if small
                  'approxval':approx
                 }
