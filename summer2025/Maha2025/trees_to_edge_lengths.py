...
CALCULATE_FRECHET_MEAN = True # set to false to avoid calculating the Frechet mean, even if it hasn't already been calculated
...
mu = (0.94*10**(-9))
# TAU1 = 100    # tauAB for both
# TAU2 = 200    # tauABC for CAT; tauCD for BAL
...
def simulate_gene_trees(tau1, tau2, tau3, N1, N2, N3, pop_size, num_gene_trees, caterpillar_species_tree, gene_tree_file):
    ...
    sp_tree = dpy.Tree.get(data = sp_tree_str, schema = "newick", preserve_underscores = True)
...
#            fout.write(gene_tree.as_string(schema='newick', preserve_underscores = True))
            fout.write(gene_tree.as_string(schema='newick').replace("_1",""))  # get rid of _1
...
#                preserve_underscores = True)
    ...
    print("here df.columns is:", df.columns)
    ...
    print("new_col_names is",new_col_names)
